...
if($_SERVER["REQUEST_METHOD"]=="POST"){
    ...
    // Try with logged_by first, fall back if column not yet added
    $stmt = $conn->prepare("INSERT INTO job_time (job_id, hours, description, logged_by) VALUES (?, ?, ?, ?)");
    if($stmt) {
        $stmt->bind_param("idsi", $id, $hours, $desc, $user_id);
    } else {
        $stmt = $conn->prepare("INSERT INTO job_time (job_id, hours, description) VALUES (?, ?, ?)");
        $stmt->bind_param("ids", $id, $hours, $desc);
    }
    $stmt->execute();
...
<title>Add Labour — Garage System</title>
<?php include "form_style.php"; ?>
</head>
<body>
<div class="page-header"><h1>Add Labour</h1></div>
<div class="form-wrap">
<div class="form-card">
    <form method="POST">
        <?php csrf_field(); ?>
